...
この突き合わせは `test/check-rule.ps1` / `test/check-rule.sh` で何度でも実行できる。規則を疑う事態になったら走らせること。不一致があれば終了コード 1 を返す。

**全件一致しても、`cwd` に現れなかった文字については何も言えない**（Windows がまさにそれで、`:` `\` `_` しか含まれていなかった）。そのためチェッカーは実際に現れた文字を最後に報告し、`.` が無ければその旨を警告する。`.` を含むディレクトリを作ってそこで一度 claude を起動すれば実例が得られる。

中身は下のとおりで、フォルダ名と `cwd` を比べているだけ。
